...
int main(void)
{
  ...
  char tryb=0;                     //tryb pracy 1-widmo w postaci tekstowej, 2-widmo w postaci graficznej
  ...
  while (1) {
    ...
    if (Transfer>0){
      ...
      if (!GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0)){ //Widmo w formie wartosci liczbowych - pierwsze 65 prazkow
        if (tryb!=1){                                 //Czyszczenie ekranu i rysowanie elementow stalych tylko raz, przy zmianie trybu
          TFTN_FillRect(  0, 15, 131, 131, 0xEEE);
     ...
     } else {                                         //Widmo w formie graficznej - 128 prazkow
        if (tryb!=2){                                 //Czyszczenie ekranu i rysowanie elementow stalych tylko raz, przy zmianie trybu
          TFTN_FillRect(  0, 15, 131, 131, 0xEEE);
          TFTN_WriteXY("0\0",     2, 100, 0x000, 0xEEE, font1 ); //opisy osi czestotliwosci          
          TFTN_FillRect(     2, 95,     2, 98, 0x00F);
          TFTN_WriteXY("125\0",  26, 100, 0x000, 0xEEE, font1 );        
          TFTN_FillRect(  32+2, 95,  32+2, 98, 0x00F);
          TFTN_WriteXY("250\0",  58, 100, 0x000, 0xEEE, font1 );        
          TFTN_FillRect(  64+2, 95,  64+2, 98, 0x00F);
          TFTN_WriteXY("375\0",  90, 100, 0x000, 0xEEE, font1 );        
          TFTN_FillRect(  96+2, 95, 96+2, 98, 0x00F);
          TFTN_WriteXY("500\0", 113, 100, 0x000, 0xEEE, font1 );        
          TFTN_FillRect(   129, 95,  129, 98, 0x00F);

          TFTN_WriteXY("[Hz]\0", 54, 110, 0x000, 0xEEE, font1 );                        
          TFTN_WriteXY("fs=1kHz\0", 44, 122, 0x000, 0xEEE, font1 );           
          tryb=2;
        }
        for (i=0; i<N/2; i++){                         //rysowanie poszczegolnych prazkow widma i kasowanie poprzednich
          TFTN_FillRect(2+i, 95,                   2+i, 95-buforFFTAmp[i]/32, 0xF00);
          TFTN_FillRect(2+i, 95-buforFFTAmp[i]/32, 2+i, 30, 0x000);
        } 
      }
    }
...
void WidmoAmp(long int buforFFT[], long int buforFFTAmp[], long n) {
  ...
  for (i=1; i < n/2; i++) {      //sumowanie prazkow dla czestotliwosci dodatniej i ujemnej
