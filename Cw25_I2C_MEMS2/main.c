...
//JP5 PB7           <-> Con3 SDA
//JP5 GND           <-> Con3 GND
//JP5 +3,3V (GPIOB) <-> Con3 +V

#include "stm32f10x.h"
#include ".\..\Moduly\LCD1602\lcd_hd44780_lib.h"
#include ".\..\Moduly\MEMS2\mems2_i2c_lib.h"
#include "stdio.h"
#include "stdlib.h"
...
int main(void)
{
  ...
  if (LIS35_I2C_Init()==0){
    GPIO_WriteBit(GPIOB, GPIO_Pin_8, SET);       //akcelerometr zainicjalizowany 
  }
  ...
  while (1) {
    ...
    LIS35_ReadRegister(LIS35_CLICK_SRC, &click);
    x+=xKor; 
    y+=yKor; 
    z+=zKor; 
...
//konfigurowanie sygnalow taktujacych
{
  ...
  if(HSEStartUpStatus == SUCCESS)
  {
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);//wlacz taktowanie portu GPIO C
  	RCC_APB1PeriphClockCmd(RCC_APB1Periph_I2C1, ENABLE); //wlacz taktowanie I2C
