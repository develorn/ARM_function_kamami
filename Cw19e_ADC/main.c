...
int main(void)
{
  ...
  unsigned long int napiecie=0, temperatura=0;
  ...
  ADC_SoftwareStartConvCmd(ADC1, ENABLE);                                    //wykonaj jeden pomiar poczatkowy, zeby mozna bylo wyswietlic pierwsze wartosci na wyswietlaczu
  ADC_ExternalTrigConvCmd(ADC1, ENABLE);                                     //wlacz trigger dla drugiego pomiaru, ktory nastapi po 2 sekundach
  while (1) {
...
void ADC_Config(void)
{
  ...
  ADC_InitStructure.ADC_ScanConvMode = ENABLE;                          //Pomiar wszystkich wlaczonych kanalow
  ...
  ADC_AnalogWatchdogThresholdsConfig(ADC1, 0x0C1E, 0x04D8);             //prog gorny 2.5V, prog dolny 1V
...
void TIM_Config(void) {
  ...
  TIM_TimeBaseStructure.TIM_Period = 20000;                             //okres przepelnienia licznika = 20000 taktow = 2 sekundy
  ...
  TIM_OCInitStructure.TIM_Pulse =20000;                                 //20000 taktow = 2s od restartu licznika
  ...
  //wlaczenie przerwania od przepelnienia / aktualizacji wartoœci Period TIM1
