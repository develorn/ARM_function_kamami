...
void TFTN_Rotate(unsigned char Rotation){
  TFTN_SendCmd(MADCTL);  //

  switch (Rotation){
    case 0: {TFTN_SendData(0x10);} break;
    case 1: {TFTN_SendData(0x60);} break;
    case 2: {TFTN_SendData(0xD0);} break;
    case 3: {TFTN_SendData(0xB0);} break;
    default: {TFTN_SendData(0x10);} break;
  }
}
...
void TFTN_PutPixel(int x, int y, unsigned int color) {
  //Set a color of a pixel in x, y position
