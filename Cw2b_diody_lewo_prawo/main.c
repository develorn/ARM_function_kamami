...
int main(void)
{
  ...
  while (1) {
    ...
    if (kierunekLewo){
      pozWlaczLED=pozWlaczLED<<1;                       //jesli w lewo, przesun pozycje w lewo
      if (pozWlaczLED>=0x8000) {                        //jesli dioda 8 - zmien kierunek
        kierunekLewo=FALSE;
      }
    } else {
      pozWlaczLED=pozWlaczLED>>1;                       //jesli w prawo, przesun pozycje w prawo
      if (pozWlaczLED<=0x0100) {                        //jesli dioda 1 - zmien kierunek
        kierunekLewo=TRUE;
      }
    }
