...
//konfigurowanie sygnalow taktujacych
{
  ...
  /*Tu nalezy umiescic kod zwiazany z konfiguracja sygnalow zegarowych potrzebnych w programie peryferiow*/
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_TIM1, ENABLE); //wlacz taktowanie TIM1
		RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2, ENABLE); //wlacz taktowanie TIM2
  } else {
  } 
}
...
void TIM_Config(void) {
  ...
  //Konfiguracja licznika 2
	//Ustawienia taktowania i trybu pracy licznika 2
	TIM_TimeBaseStructure.TIM_Prescaler = 7200-1;                //taktowanie licznka fclk = 72MHz/7200 = 10kHz
  TIM_TimeBaseStructure.TIM_Period = 20000;                    //okres przepelnien licznika = 20000 taktow = 2 sekundy
	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;      //dzielnik zegara dla ukladu generacji dead-time i filtra
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;  //tryb pracy licznika

	TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure);              //Inicjalizacja licznika
	...
	TIM_OCInitStructure.TIM_Pulse =5000;                         //5000 taktow = 0.5s od restartu licznika
	TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High;    //polaryzacja wyjscia
	TIM_OC1Init(TIM2, &TIM_OCInitStructure);                     //Inicjalizacja kanalu 1

	TIM_OC1PreloadConfig(TIM2, TIM_OCPreload_Disable);           //zapis ustawien, wylaczenie buforowania  - ewentualne zmiany konfiguracji uwzgledniane sa od razu 
                                                               //gdy preload jest wlaczony - zmiany wchodza w zycie dopiero przy nastepnym update
  ...
  TIM_ITConfig(TIM2, TIM_IT_Update , ENABLE);  //wlaczenie przerwania od przepelnienia
  TIM_ITConfig(TIM2, TIM_IT_CC1 , ENABLE);     //wlaczenie przerwanie od porownania w kanale 1
