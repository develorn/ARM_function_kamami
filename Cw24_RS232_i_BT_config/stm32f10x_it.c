...
void USART1_IRQHandler(void)
{
  ...
  //if((USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  && (odebranoDane==FALSE))
  if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET) 
  {
    buforRx[bufRxIndex] = USART_ReceiveData(USART1); //Odczyt danej automatycznie kasuje flage przerwania           
    if(buforRx[bufRxIndex] == 0x0A) {                //Wykrycie znaku \n
      odebranoDane = TRUE;
      while (bufRxIndex<22) {                        //Wyzerowanie znakow do konca bufora  (zabezpiecza przed wyswietlaniem "smieci" na LCD)
        buforRx[bufRxIndex]=0;      
        bufRxIndex++;
      }
      bufRxIndex = 0;
    } else {                                         //Obsluga pozostalych, "zwyklych" znakow
      if (buforRx[bufRxIndex] != 0x0D) {             //Ignorowanie znaku \r
        bufRxIndex++;
        if (bufRxIndex>22){                          //Zabezpieczenie przed przepelnieniem bufora jesli odbierany tekst jest dluzszy niz 16 znakow
          bufRxIndex=0;
        }
      }
    }
  ...
  if(USART_GetITStatus(USART1, USART_IT_TXE) != RESET) {   
    ...
    for (i=0; i<150000; i++);                         //Odczekanie 10ms pomiedzy kolejnymi znakami komendy
