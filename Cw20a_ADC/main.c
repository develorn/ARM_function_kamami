...
int main(void)
{
  ...
  LCD_WriteTextXY("ADC1:  0,000 V\0",0,0);            
  LCD_WriteTextXY("ADC2:  0,000 V\0",0,1);            
...
//    //wartoscADC1 = ADC_GetConversionValue(ADC1) * 0.8059;                           //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
    //wartoscADC1 = ADC_GetConversionValue(ADC1) * 8059/10000;                       //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
 	  //sprintf((char *)wartoscADC1Tekst, "%d,%03d V", wartoscADC1 / 1000, wartoscADC1 % 1000);//Dzielenie calkowite wyzancza wartosc w V,  dzielenie modulo - czeasc po przecinku
    //LCD_WriteTextXY(wartoscADC1Tekst,6,0);         
    ...
    while (!ADC_GetFlagStatus(ADC2,ADC_FLAG_EOC));                                 //odczekaj na zakonczenie konwersji
      //wartoscADC2V = ADC_GetConversionValue(ADC2) * 0.8059;   //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
		  wartoscADC2  = ADC_GetConversionValue(ADC2);             //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
		  wartoscADC2V =  wartoscADC2 * 8059/10000;                //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
	  	sprintf((char *)wartoscADC2VTekst, "%d,%03d V\0", wartoscADC2V / 1000, wartoscADC2V % 1000);//Przekszta³cenie wyniku na tekst, dzielenie calkowite wyzancza wartosc w V, dzielenie modulo - czesc po przecinku
      LCD_WriteTextXY(wartoscADC2VTekst,7,1);


//    //wartoscADC2 = ADC_GetConversionValue(ADC2) * 0.8059;                          //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
    //wartoscADC2 = ADC_GetConversionValue(ADC2) * 8059/10000;                       //przelicz wartosc wyrazona jako calkowita, 12-bit na rzeczywista
    //sprintf((char *)wartoscADC2Tekst, "%d,%03d V", wartoscADC2 / 1000, wartoscADC2 % 1000);//Dzielenie calkowite wyzancza wartosc w V,  dzielenie modulo - czeasc po przecinku
    //LCD_WriteTextXY(wartoscADC2Tekst,6,1);         
...
void ADC_Config(void)
{
  ...
  ADC_InitStructure.ADC_Mode = ADC_Mode_RegSimult;                    //Dwa przetworniki przetwornik, praca jednoczesna
  ADC_InitStructure.ADC_ScanConvMode = DISABLE;                       //Pomiar jednego kanalu, skanowanie kanalow nie potrzebne
  ADC_InitStructure.ADC_ContinuousConvMode = ENABLE;                  //Pomiar w trybie ciaglym
  ADC_InitStructure.ADC_ExternalTrigConv = ADC_ExternalTrigConv_None;	//Brak wyzwalania zewnetrznego
  ADC_InitStructure.ADC_DataAlign = ADC_DataAlign_Right;            	//Wyrownanie danych do prawej - 12 mlodszych bitow znaczacych
